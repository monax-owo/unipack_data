...
# if special key
# enter
execute as @e[limit=1, sort=nearest, tag=cashier] if score @s _cashier_10key_input matches -99 run scoreboard players set @e[limit=1, sort=nearest, tag=cashier] _cashier_10key_count 0
# del
execute as @e[limit=1, sort=nearest, tag=cashier] if score @s _cashier_10key_input matches -100 run say backspace

tellraw @a ["count: ", {score: {name: "@e[limit=1, sort=nearest, tag=cashier]", objective: "_cashier_10key_count"}}]
